...
class DataModel: ObservableObject {
  ...
  init() {
    ...
    if url.lastPathComponent != "COMMIT_EDITMSG" {
      ...
      AppMode = .commit
      self.commit.url = url
      self.repo = try? GitRepository(atPath: url.pathComponents.dropLast(2).joined(separator: "/"))
  ...
  func setError(title: String = "Error", body: String, status: String, kind: ApplicationStatusKind = .none) {
